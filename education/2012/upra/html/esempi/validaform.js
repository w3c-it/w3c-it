...
function validateNumbers(objForm)
{
  ...
  if (!bFirst || !bSecond)
  {
    ...
    objAnchor.appendChild(document.createTextNode('Errori nella sottomissione del form'));
    objAnchor.setAttribute('href', '#firsterror');
    objTitle.appendChild(objAnchor);
    objParagraph.appendChild(document.createTextNode('Prego, ricontrollare i seguenti'));
    ...
    if (!bFirst)
    {
      strError = 'Prego, fornire un valore numerico per il primo numero';
      objList.appendChild(addError(strError, '#num1', objForm, strID));
      strID = '';
    }
    if (!bSecond)
    {
      strError = 'Prego, fornire un valore numerico per il secondo numero';
