...
task :deploy, [:deployment_configuration] => :build do |t, args|
  ...
  if matchdata
    ...
    # determine if we are trying to deploy on a non-standard port
    portdata = text.match(/^port: (.*)$/)
    port = "--port=#{portdata[1]}" if portdata

    sh "rsync #{port} -avz --delete _site/ #{deploy_dir}"
    time = Time.new
